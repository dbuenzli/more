...
let test_name =
  Test.test "Os.{Name,Version,Arch}.get" @@ fun () ->
  let name = Os.Name.get () in
  let name_like = Os.Name.get ~id_like:true () in
  let version = Os.Version.get () in
  let arch = Os.Arch.get () in
