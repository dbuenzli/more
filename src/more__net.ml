...
(* Note this was copied back from affect. *)
...
let socket ~nonblock endpoint socket_type =
...
let unix_close_noerr fd = try Unix.close fd with Unix.Unix_error _ -> ()
...
module Endpoint = struct
  ...
  let err_wait ep e = More__fmt.str "Wait on %a: %s" pp ep e
  let wait_connectable' ?(socket_type = Unix.SOCK_STREAM) ~timeout ep =
    let relax = More__os.sleep More__mtime.Span.(1 * ms) in
    Result.map_error (err_wait ep) @@
    let rec loop ~deadline dur =
      let* addr, fd = socket ~nonblock:false ep socket_type in
      let close = match ep with `Fd _ -> false | _ -> true in
      let finally () = if close then unix_close_noerr fd in
      ...
      | `Ready -> Ok `Ready
      | `Retry ->
          let count = More__os.Mtime.count dur in
          if More__mtime.Span.is_shorter count ~than:deadline
          then (ignore (More__os.sleep relax); loop ~deadline dur)
          else Ok `Timeout
    in
    loop ~deadline:timeout (More__os.Mtime.counter ())
...
module Connection = struct
  ...
  let open_exn ?(nonblock = false) ?socket_type ~peer () = match peer with
  ...
  let close_noerr c =
    ...
    (match c.peer with `Fd _ -> () | _ -> unix_close_noerr c.fd)
...
module Listener = struct
  ...
  let open_exn ?(nonblock = false) ?socket_type ?(backlog = 128) ~endpoint () =
    match endpoint with
