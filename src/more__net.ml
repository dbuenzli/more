...
module Endpoint = struct
  ...
  let of_string ~default_port s =
    ...
    | Some i ->
        ...
        | None ->
            let h = More__string.subrange ~last:(i - 1) s in
            let p = More__string.subrange ~first:(i + 1) s in
            match int_of_string_opt p with
            | None -> More__fmt.error "port %S not an integer" p
            | Some p -> Ok (`Host (h, p))
  ...
  let pp ppf = function
  | `Host (n, p) -> More__fmt.host_and_port ppf (n, p)
  | `Fd _fd -> More__fmt.pf ppf "<fd>"
  | `Sockaddr addr -> More__fmt.sockaddr ppf addr
end
