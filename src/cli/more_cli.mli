...
(** Cmdliner support for [more] programs.
    ...
    The {{!page-cookbook.blueprints}cookbook blueprints} has some examples. *)
...
(** Program exits.
    ...
    Support for {!More.Os.Exit} exits which are more evolved
...
(** [cmd] is a converter for commands which are parsed using
    {!More.Cmd.of_string}. Completes files. *)
...
(** [set_no_color ()] behaves like {!no_color} and sets
    {!More.Fmt.val-styler} to [Plain] when it's [true]. See
    {{!page-cookbook.blueprint_color_log}an example}. *)
...
(** [set_log_level] behaves like {!val-log_level} and sets the log level.
    See {{!page-cookbook.blueprint_color_log}an example}. *)
...
(** [net_endpoint_conv ~default_port] is a converter for socket
    ...
    strings with {!More.Net.Endpoint.of_string}. *)
