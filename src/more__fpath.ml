...
let err_invalid_seg s =
  More__fmt.str "%a: Invalid path segment" More__fmt.OCaml.string s

let err_start s =
  More__fmt.error "%a: Not a path" More__fmt.OCaml.string s

let err_null s =
  More__fmt.error "%a: Not a path: has null bytes" More__fmt.OCaml.string s

let err_empty s =
  More__fmt.error "%a: Not a path: is empty" More__fmt.OCaml.string s

let err_ext_root s ~ext =
  More__fmt.str "Cannot append extension %s to root path %s" ext s
...
| '%' | '#' | '?' -> 3
| ' ' when escape_space -> 3
| c when More__char.Ascii.is_control c -> 3
| _ -> 1

let set_pct_encoded b i c =
  let c = Char.code c in
  let hi = More__char.Ascii.upper_hex_digit_of_int (c lsr 4) in
  let lo = More__char.Ascii.upper_hex_digit_of_int (c      ) in
...
| c when More__char.Ascii.is_control c -> set_pct_encoded b i c
...
module Windows = struct
  ...
  let with_volume vol p = vol ^ (More__string.subrange ~first:(path_start p) p)
  let cut_volume p = More__string.cut_first (path_start p) p
  let drop_volume p = More__string.drop_first (path_start p) p
  let take_volume p = More__string.take_first (path_start p) p
  ...
  let to_url_path ?(escape_space = true) p =
    ...
    in
    let p = More__string.byte_escaper (pct_esc_len ~escape_space) set_char p in
...
module Posix = struct
  ...
  let drop_volume p = More__string.subrange ~first:(path_start p) p
  let take_volume p =
    More__string.subrange ~first:2 (* skip // *) ~last:(path_start p - 1) p

  let cut_volume p =
    let path_start = path_start p in
    if path_start = 0 then "", p else
    (More__string.subrange ~first:2 (* skip // *) ~last:(path_start - 1) p,
     More__string.subrange ~first:path_start p)

  let with_volume vol p =
    if vol = "" then p else
    if not (is_segment vol) then invalid_arg (err_invalid_seg vol) else
    let start = path_start p in
    let q = if start = 0 then p else More__string.subrange ~first:start p in
  ...
  let to_url_path ?(escape_space = true) p =
    ...
    More__string.byte_escaper esc_len (pct_esc_set_char ~escape_space) p
end
...
let fmt fmt = More__fmt.kstr v fmt
...
| exception Not_found -> More__string.subrange ~first:(path_start p) p
| sep -> More__string.subrange ~first:(sep + 1) p
...
| segs ->
    ...
    let p = (* on POSIX empty segs can make [v] parse a volume, drop it *)
      ...
      && p.[1] = natural_dir_sep_char then More__string.drop_first 1 p else p
    in
...
let drop_root_sep p =
  ...
  if not Sys.win32 then
    ...
    More__string.drop_first (path_start + 1) p
  else
  let volume, path = cut_volume p in
  with_volume volume (More__string.drop_first 1 path)
...
| 2 ->
    ...
    then More__string.of_char p.[0]
    else p
| len ->
    let max = len - 1 in
    if p.[max] <> natural_dir_sep_char then p else
    More__string.subrange p ~last:(max - 1)
...
let drop_strict_prefix ~prefix:pre p =
  ...
  Some (More__string.subrange p ~first)
...
| exception Not_found -> ""
| first, last -> More__string.subrange ~first ~last p

let rem_ext' efirst elast p =
  let plen = String.length p in
  match elast = plen - 1 with
  | true -> More__string.subrange ~last:(efirst - 1) p
  | false ->
...
| exception Not_found -> p, ""
| efirst, elast ->
    let ext = More__string.subrange ~first:efirst ~last:elast p in
...
let rec parent p =
  ...
  | 1 when p.[seg_first] = '.' ->
      ...
      parent (More__string.subrange ~last:(seg_first - 1) p)
  | 2 when p.[seg_first] = '.' && p.[seg_last] = '.' ->
      let via_dotdot p = append_seg' p ".." in
      via_dotdot p
  | _ when seg_first = path_start -> "."
  | _ -> append_seg' (More__string.subrange ~last:(seg_first - 1) p) ""
...
let relative ~to_dir p =
  ...
  if More__string.includes
      ~affix:".." to_dir (* cmon that's obvi..ously wrong *)
  then More__fmt.invalid_arg "%s: no dotdot allowed" p;
...
let pp_quoted ppf p = More__string.pp ppf (Filename.quote p)
let pp_unquoted = More__string.pp
let pp ppf p =
  if String.exists (Char.equal ' ') p
  then pp_quoted ppf p
  else More__string.pp ppf p

let pp_dump = More__fmt.OCaml.string

let error p fmt = More__fmt.error ("%a: " ^^ fmt) pp_unquoted p
let prefix_msg p msg = More__fmt.str "%a: %s" pp_unquoted p msg

(* Uniqueness *)

let distinct = More__string.distinct
...
module Set = struct
  let pp_set ppf ss =
    More__fmt.pf ppf "@[<1>{%a}@]" (More__string.Set.pp ~sep:More__fmt.sp pp) ss

  include More__string.Set
end
module Map = More__string.Map
...
let sort_by_ext ~multi ps =
  let add_path p acc =
    More__string.Map.add_to_set (module Set) (take_ext ~multi p) p acc
  in
  Set.fold add_path ps More__string.Map.empty
...
let list_of_search_path ?(sep = search_path_sep) path =
  ...
  | ""  -> Ok (List.rev acc)
  | rest ->
      let dir, rest = match More__string.split_first ~sep rest with
      | None -> rest, ""
      ...
      | Error e -> More__fmt.error "Illegal path %a in search path: %s" pp dir e
  in
  loop [] path
