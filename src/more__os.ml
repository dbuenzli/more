...
let path_msg p e =  More__fmt.str "%a: %s" More__filepath.pp p e
let op_path_msg' op path e = More__fmt.str "%s %a: %s" op More__fmt.code path e
let op_path_msg op path e =
  More__fmt.str "%s %a: %s" op (More__fmt.code' More__filepath.pp) path e
...
module Fs_base = struct
  ...
  let dir_create ?(mode = 0o755) ~make_path dir =
    ...
    let mkdir dir mode = Unix.mkdir (More__filepath.to_string dir) mode in
    try
      let pmode = 0o755 in
      try Ok (mkdir dir mode; true) with
      | Unix.Unix_error (EEXIST, _, _) ->
          if is_dir (More__filepath.to_string dir) then Ok false else
      ...
      | Unix.Unix_error (ENOENT, _, _) when make_path ->
          ...
          | exception Unix.Unix_error (ENOENT, _, _) ->
              up (p :: todo) (More__filepath.parent p)
          in
          up [dir] (More__filepath.parent dir)
    with
    | Unix.Unix_error (e, _, p) ->
        if String.equal (More__filepath.to_string dir) p
  ...
  let dir_delete ~recurse dir =
    ...
    let fail e = More__fmt.failwith_notrace "%a: %s" More__filepath.pp dir e in
    let rec delete_symlink p =
      if is_symlink p then (unlink p; true) else false
    in
    let try_unlink file = match Unix.unlink (More__filepath.to_string file) with
    | () -> true
    ...
    | file ->
        let file = More__filepath.(d / file) in
    ...
    let rec try_delete d todo =
      match Unix.opendir (More__filepath.to_string d) with
      | dh ->
    ...
    | [] -> ()
    | d :: ds ->
        match Unix.rmdir (More__filepath.to_string d) with
        | () -> doit ds
        | exception Unix.Unix_error (e, _, _) ->
            match e with
            ...
            | e -> fail (op_path_msg delete_op d (uerror e))
    in
    try match Unix.rmdir (More__filepath.to_string dir) with
    | () -> Ok true
    | exception Unix.Unix_error (e, _, _) ->
        match e with
        ...
        | ENOTDIR as e ->
            begin try
              if delete_symlink (More__filepath.to_string dir) then Ok true else
  ...
  let rec handle_force ~force file =
    ...
    try
      ignore (Unix.lstat (More__filepath.to_string file));
  ...
  let rec handle_force_open_fdout
    ...
    match Unix.openfile (More__filepath.to_string file) fls mode with
    | fd -> Ok fd
    | exception Unix.Unix_error (EINTR, _, _) ->
        handle_force_open_fdout ~flags ~force ~make_path ~mode file
    | exception Unix.Unix_error (ENOENT as e, _, _) when make_path ->
        begin match dir_create ~make_path (More__filepath.parent file) with
        | Ok true (* created *) ->
            handle_force_open_fdout ~flags ~force ~make_path ~mode file
        | Ok false (* existed *) ->
  ...
  let rec path_exists p =
    try (ignore (Unix.stat (More__filepath.to_string p)); Ok true) with
  ...
  let rec path_exists_stat p =
    try Ok (Some (Unix.stat (More__filepath.to_string p))) with
  ...
  let rec path_get_mode p =
    ...
    try Ok ((Unix.stat (More__filepath.to_string p)).Unix.st_perm) with
  ...
  let rec path_rename src ~force ~make_path ~dst:p =
    let err e =
      More__fmt.str "rename %a to %a: %s" More__filepath.pp src More__filepath.pp p e
    in
    ...
    | Ok () ->
        try
          Ok (Unix.rename
                (More__filepath.to_string src) (More__filepath.to_string p))
        with
        | Unix.Unix_error (ENOENT as e, _, _) when make_path ->
            begin match dir_create ~make_path (More__filepath.parent p) with
  ...
  let rec path_stat p = try Ok (Unix.stat (More__filepath.to_string p)) with
  ...
  let rec symlink ~src ~force ~make_path p =
    let err e =
      More__fmt.str "symlink %a to %a: %s"
        More__filepath.pp src More__filepath.pp p e
    in
    try
      Ok (Unix.symlink
            (More__filepath.to_string src)
            (More__filepath.to_string p))
    with
    | Unix.Unix_error (EEXIST, _, _) when force ->
        begin match unlink (More__filepath.to_string p) with
        | exception Unix.Unix_error (ENOENT, _, _) ->
  ...
  let rec symlink_link p =
    try
      let l = Unix.readlink (More__filepath.to_string p) in
      match More__filepath.of_string l with
      | Ok _ as v -> v
  ...
  let rec symlink_stat p = try Ok (Unix.lstat (More__filepath.to_string p)) with
...
module Tmp = struct
  let rec atomic_add set el =
    ...
    if Atomic.compare_and_set set seen (More__filepath.Set.add el seen)
    then () else atomic_add set el

  let rec atomic_remove set el =
    let seen = Atomic.get set in
    if Atomic.compare_and_set set seen (More__filepath.Set.remove el seen)
    then () else atomic_remove set el

  let delete_file file = try Fs_base.unlink (More__filepath.to_string file) with
  | Unix.Unix_error (e, _, _) -> ()

  let files = Atomic.make More__filepath.Set.empty
  ...
  let dirs = Atomic.make More__filepath.Set.empty
  let add_dir dir = atomic_add dirs dir
  let rem_dir dir = delete_dir dir; atomic_remove dirs dir

  let cleanup () =
    More__filepath.Set.iter delete_file (Atomic.get files);
    More__filepath.Set.iter delete_dir (Atomic.get dirs)
  ...
  let default_dir =
    ...
    in
    ref (More__filepath.ensure_trailing_dir_sep (More__filepath.v dir))
  ...
  let tmp_path dir name rand =
    let dir = More__filepath.to_string dir in
    match dir.[String.length dir - 1] = More__filepath.natural_dir_sep_char with
    | true -> Printf.sprintf ("%s" ^^ name) dir rand
    | false ->
        Printf.sprintf ("%s%c" ^^ name)
          dir More__filepath.natural_dir_sep_char rand
  ...
  let open'
    ...
    let dir = More__filepath.ensure_trailing_dir_sep dir in
    ...
    | n ->
        ...
        try
          ...
          let file = More__filepath.v file in
  ...
  let mkdir
    ...
    | n ->
        ...
        try
          ...
          let tdir = More__filepath.v tdir in
  ...
  let path ?(make_path = true) ?dir ?(name = format_of_string "tmp-%s") () =
    ...
    | n ->
        ...
        | exception Unix.Unix_error (ENOENT, _, _) -> Ok (More__filepath.v file)
...
module File = struct
  ...
  let rec exists file =
    match (Unix.stat (More__filepath.to_string file)).Unix.st_kind with
    | Unix.S_REG -> Ok true
    | _ -> Ok false
  ...
  let is_executable file = is_executable' (More__filepath.to_string file)
  ...
  let rec delete path =
    ...
    | exception Unix.Unix_error (e, _, _) ->
        Error (op_path_msg' op p (uerror e))
    in
    let p = More__filepath.to_string path in
  ...
  let rec truncate file size =
    try Ok (Unix.truncate (More__filepath.to_string file) size) with
  ...
  let rec link ~src ~force ~make_path file =
    let err e =
      More__fmt.error "link %a to %a: %s"
        More__filepath.pp src More__filepath.pp file e
    in
    try
      Ok (Unix.link
            (More__filepath.to_string src)
            (More__filepath.to_string file))
    with
    | Unix.Unix_error (EEXIST, _, _) when force ->
        begin match Fs_base.unlink (More__filepath.to_string file) with
        | exception Unix.Unix_error (ENOENT, _, _) ->
    ...
    | Unix.Unix_error ((ENOENT | ENOTDIR as e), _, _)
      when make_path ->
        begin
          match Fs_base.dir_create ~make_path (More__filepath.parent file) with
  ...
  let read_with_ic file f =
    try
      let ic =
        if More__filepath.is_dash file
        then (In_channel.set_binary_mode In_channel.stdin true; stdin)
        else open_in_bin (More__filepath.to_string file)
      in
  ...
  let read_with_fd
    ...
    try
      let fd =
        if More__filepath.is_dash file then Unix.stdin else
        Fd.openfile (More__filepath.to_string file) flags 0
      in
  ...
  let rec rename_tmp src dst =
    try
      Ok (Unix.rename
            (More__filepath.to_string src)
            (More__filepath.to_string dst))
    with
    | Unix.Unix_error (EINTR, _, _) -> rename_tmp src dst
    | Unix.Unix_error (e, _, _) ->
        Error
          (More__fmt.str "renaming %a to %a: %s"
            More__filepath.pp src More__filepath.pp dst (uerror e))
  ...
  let write_with_fd_atomic ?flags ~mode ~force ~make_path ~file f =
    ...
    in
    let dir = More__filepath.parent file in
  ...
  let write_with_fd
    ...
    =
    if More__filepath.is_dash file then Ok (f Unix.stdout) else
    if atomic
  ...
  let copy ?atomic ?mode src ~force ~make_path ~dst =
    let error e =
      More__fmt.str "copy %a to %a: %s"
        More__filepath.pp src More__filepath.pp dst e
    in
    ...
    try
      ...
      | None -> Some (Unix.stat (More__filepath.to_string src)).Unix.st_perm
      in
...
module Dir = struct
  ...
  let rec readdir ~dotfiles dir =
    ...
    | n when More__filepath.is_segment n -> loop ~dotfiles dir dh (n :: acc)
    | n ->
        More__fmt.failwith "%a: Invalid filename %a"
          More__filepath.pp dir More__fmt.OCaml.string n
    in
    let dh = Unix.opendir (More__filepath.to_string dir) in
  ...
  let rec exists dir =
    match (Unix.stat @@ More__filepath.to_string dir).Unix.st_kind with
    | Unix.S_DIR -> Ok true
    | _ -> Ok false
    | exception Unix.Unix_error ((ENOENT|ENOTDIR), _, _) -> Ok false
  ...
  let rec delete ~recurse dir =
    ...
    | exception Unix.Unix_error (e, _, _) ->
        Error (op_path_msg' op p (uerror e))
    in
    let p = More__filepath.to_string dir in
    match (Unix.lstat p).Unix.st_kind with
  ...
  let rec stat p = try (Unix.stat @@ More__filepath.to_string p) with
  | Unix.Unix_error (EINTR, _, _) -> stat p

  let rec lstat p = try (Unix.lstat @@ More__filepath.to_string p) with
  ...
  let fold_no_rec
    ...
    | [] -> Ok acc
    | n :: ns ->
        let full = More__filepath.(adir / n) in
        ...
        | st ->
            ...
            | Unix.S_DIR ->
                ...
                let p = if rel then More__filepath.v n else full in
        ...
        | exception Unix.Unix_error (e, _, _) ->
            stat_error (if not rel then full else More__filepath.v n) e;
            loop stat_error stat f acc adir ns
    in
    ...
    | n :: ns ->
        ...
        | st ->
            ...
            | Unix.S_DIR ->
                ...
                | None -> More__filepath.v n
                | Some rdir -> More__filepath.(rdir / n)
                in
  ...
  let warn_and_prune_denied ~dir ~rel _ _ p _ =
    let ap = if rel then More__filepath.(dir // p) else p in
    try (Unix.access (More__filepath.to_string ap) Unix.[R_OK; X_OK]; false)
    with
    | Unix.Unix_error ((EACCES | EPERM (* may happen *)), _, _) ->
        More__log.warn (fun m -> m "%a: Permission denied" More__filepath.pp p);
        true

  let warn_stat_error p e =
    More__log.warn (fun m -> m "%a: %s" More__filepath.pp p (uerror e))
  ...
  let _fold
    ...
    | Unix.Unix_error (e, _, ep) ->
        let err = uerror e in
        let err =
          if String.equal (More__filepath.to_string dir) ep then err else
  ...
  | Unix.S_DIR -> More__filepath.ensure_trailing_dir_sep f :: acc
  | _ -> f :: acc
  ...
  let copy
    ...
    | Unix.S_REG ->
        ...
        let mode = st.Unix.st_perm in
        let src = More__filepath.(src // p) in
        let dst = More__filepath.(dst // p) in
    ...
    | _ ->
        More__fmt.failwith "%a: Not a regular file, directory or symlink"
          More__filepath.pp More__filepath.(src // p)
    in
    ...
    let* tdst =
      if atomic
      then Tmp.mkdir ~make_path ~dir:(More__filepath.parent dst) ()
  ...
  let set_default_tmp p =
    Tmp.default_dir := More__filepath.ensure_trailing_dir_sep p
  ...
  let rec cwd () =
    ...
    match More__filepath.of_string (Unix.getcwd ()) with
    | Ok dir when More__filepath.is_absolute dir -> Ok dir
    | Ok dir -> err (More__fmt.str "%a is relative" More__filepath.pp dir)
    | Error e -> err e
    | exception Unix.Unix_error (EINTR, _, _) -> cwd ()
    | exception Unix.Unix_error (e, _, _) -> err (uerror e)

  let rec set_cwd dir =
    let err e = More__fmt.error "set cwd to %a: %s" More__filepath.pp dir e in
    try Ok (Unix.chdir (More__filepath.to_string dir)) with
  ...
  | None -> None
  | Some p ->
      match More__filepath.of_string p with
  ...
  let user () =
    ...
    match
      ...
      | Some id ->
          ...
          | Some id -> id
      in
      More__filepath.of_string (Unix.getpwuid uid).Unix.pw_dir
    with
    | Ok _ as v -> v
  ...
  let data_fallback () =
    home_fallback data_dir (More__filepath.v ".local/share")
  let data () =
  ...
  let cache_fallback () = home_fallback cache_dir (More__filepath.v ".cache")
  let cache () =
  ...
  let state_fallback () =
    home_fallback state_dir (More__filepath.v ".local/state")
  let state () =
...
module Path = struct
  ...
  let rec must_exist p =
    try Ok (ignore (Unix.stat (More__filepath.to_string p))) with
  ...
  let rec realpath p =
    try More__filepath.of_string (Unix.realpath (More__filepath.to_string p))
    with
  ...
  let rec exists_realpath p =
    try
      ...
      More__filepath.of_string (Unix.realpath (More__filepath.to_string p))
    with
  ...
  let copy
    ...
    in
    let* src_stat =
      if More__filepath.is_dash src then Ok None else
      match Fs_base.path_stat src with
      | Ok stat -> Ok (Some stat)
      | Error e -> Error (err e)
    in
    let* dst =
      if More__filepath.is_dash dst then Ok dst else
      match Fs_base.path_exists_stat dst with
      | Error e -> Error (err e)
      | Ok Some { st_kind = S_DIR } -> Ok More__filepath.(dst / basename src)
      | Ok _ -> Ok dst
    in
    match src_stat with
    | Some { st_kind = S_DIR } ->
  ...
  let is_mount_point p =
    ...
    | Error e -> Error (err e)
    | Ok stat ->
        match Fs_base.path_stat More__filepath.(p / "..") with
...
module Cmd = struct
  ...
  let tool_is_path t = String.exists More__filepath.is_dir_sep_char t
  let tool_file ~dir tool = match dir.[String.length dir - 1] with
  | c when More__filepath.is_dir_sep_char c -> dir ^ tool
  | _ -> String.concat More__filepath.natural_dir_sep [dir; tool]
  ...
  | p ->
      ...
      | "" ->
          Error (`Dirs
                   (String.split_on_char More__filepath.search_path_sep.[0] p))
      | p ->
          let dir, p =
            let sep = More__filepath.search_path_sep in
          ...
          | false -> loop tool p
          | true -> Ok (More__filepath.v tool_file)
      in
      loop tool p

  let search_in_dirs ~dirs tool =
    let rec loop tool = function
    | [] -> Error (`Dirs (List.map More__filepath.to_string dirs))
    | d :: dirs ->
        let tool_file = tool_file ~dir:(More__filepath.to_string d) tool in
        match File.is_executable' tool_file with
  ...
  let path_search ?(win_exe = Sys.win32) ?path () = fun cmd ->
    ...
    | Some tool ->
        let tool =
          let tool = More__filepath.to_string tool and suffix = ".exe" in
        ...
        | true ->
            ...
            then Ok (More__cmd.set_tool (More__filepath.v tool) cmd)
  ...
  let in_null = In_file More__filepath.null
  ...
  | Out_file of
      { mode : int; force : bool; make_path : bool; file : More__filepath.t }
  ...
  let out_null = out_file ~force:true ~make_path:false More__filepath.null
  ...
  type spawn_tracer =
    pid option -> Env.assignments option -> cwd:More__filepath.t option ->
    More__cmd.t -> unit
  ...
  | None -> getcwd ()
  | Some d -> More__filepath.to_string d
  ...
  let run_status_out
    ...
    try
      ...
      let out = Fd.read_file (More__filepath.to_string tmpf) fd in
  ...
  let execv ?env ?cwd ?argv0 cmd =
    ...
    | file :: args as all ->
        ...
        try
          ...
          | Some cwd ->
            let old_cwd = getcwd () in
            chdir (More__filepath.to_string cwd);
...
module Name = struct
  ...
  let darwin_info uname_sysname =
    ...
    match File.read (More__filepath.v plist) with
  ...
  let etc_os_release_info uname_sysname =
    ...
    match File.read (More__filepath.v info) with
    | Error e ->
