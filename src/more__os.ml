...
let path_msg p e =  More__fmt.str "%a: %s" More__fpath.pp p e
let op_path_msg' op path e = More__fmt.str "%s %a: %s" op More__fmt.code path e
let op_path_msg op path e =
  More__fmt.str "%s %a: %s" op (More__fmt.code' More__fpath.pp) path e

(* Monotonic time *)

module Mtime = struct
  external mtime_now_ns : unit -> Int64.t = "ocaml_more_monotonic_now_ns"

  (* Monotonic clock *)

  let origin = mtime_now_ns ()
  let now () = More__mtime.of_uint64_ns (mtime_now_ns ())
  let elapsed () =
    More__mtime.Span.of_uint64_ns (Int64.sub (mtime_now_ns ()) origin)

  (* Monotonic time counter *)

  type counter = More__mtime.t
  let counter () = More__mtime.of_uint64_ns (mtime_now_ns ())
  let count c =
    More__mtime.Span.of_uint64_ns
      (Int64.sub (mtime_now_ns ()) (More__mtime.to_uint64_ns c))

  (* Sleep *)

  let sleep dur =
    let measure = counter () in
    let dur = More__mtime.Span.to_float_s dur in
...
let time ?(level = More__log.Info) m f =
  let time = Mtime.counter () in
  let v = f () in
  let span = Mtime.count time in
  More__log.kmsg (fun () -> v) level
    (fun w ->
       let header = Format.asprintf "%a" More__mtime.Span.pp span in
       m v (w ~header))

let () = More__log.set_time_func { time = time }
...
module Fd = struct
  ...
  let set_raw_mode_unix fd =
    ...
    with Unix.Unix_error (e, _, _) ->
      Error (More__fmt.str "Cannot set fd in raw mode: %s" (uerror e))
...
module Socket = struct
  ...
  | exception Unix.Unix_error (e, _, _) ->
      More__fmt.error "Connect to %a: %s" More__fmt.sockaddr addr (uerror e)
  ...
  | `Host (name, port) ->
      ...
      | exception Not_found -> More__fmt.error "%s: Host not found" name
      | h ->
  ...
  let connect_endpoint ?nonblock ep stype =
    Result.map_error (fun e -> More__fmt.str "Connect endpoint: %s" e) @@
  ...
  let listen_endpoint ?nonblock ?backlog ep stype =
    ...
    | _ -> Ok (fd, close, addr)
    in
    Result.map_error (fun e -> More__fmt.str "Listen endpoint: %s" e) @@
    match for_endpoint ?nonblock ep stype with
    | Error _ as e -> e
    | Ok (fd, true, (Some addr)) ->
  ...
  let err_wait ep e = More__fmt.str "Wait on %a: %s" More__net.Endpoint.pp ep e
  let endpoint_wait_connectable ?(socket_type = Unix.SOCK_STREAM) ~timeout ep =
    let relax = Mtime.sleep More__mtime.Span.(1 * ms) in
    ...
    let rec loop ~deadline dur =
      ...
      | `Retry ->
          ...
          if More__mtime.Span.is_shorter count ~than:deadline
  ...
  let endpoint_wait_connectable' ?socket_type ~timeout ep =
    ...
    | Ok `Timeout ->
        let err =
          More__fmt.str "Timed out after %a" More__mtime.Span.pp timeout
        in
...
module Env = struct
  ...
  let normalize = if Sys.win32 then More__string.Ascii.uppercase else Fun.id
  ...
  | Some v ->
      match parse v with
      | Error e -> More__fmt.error "%s env: %s" name e
      | Ok v -> Ok (Some v)
  ...
  let env_err e = More__fmt.error "process environment: %s" e
  ...
  let pp_assignments = More__fmt.(vbox @@ list string)

  (* Process environment *)

  type binding = var_name (* not normalized *) * string
  type t = binding More__string.Map.t (* map keys are normalized *)

  let empty = More__string.Map.empty
  let fold f env acc =
    More__string.Map.fold (fun _ (n, v) acc -> f n v acc) env acc

  let find var env =
    Option.map snd (More__string.Map.find_opt (normalize var) env)

  let add var v env = More__string.Map.add (normalize var) (var, v) env
  let remove var env = More__string.Map.remove (normalize var) env
  let mem var env = More__string.Map.mem (normalize var) env
  let override env ~by =
    if More__string.Map.is_empty by then env else
    let lean_right _ l r = match r with
    | Some _ as v -> v
    | None -> match l with Some _ as v -> v | None -> assert false
    in
    More__string.Map.merge lean_right env by

  let parse_assignments ?(init = More__string.Map.empty) fold v =
    try
      let add acc assign = match More__string.split_first ~sep:"=" assign with
...
module Fs_base = struct
  ...
  let dir_create ?(mode = 0o755) ~make_path dir =
    ...
    let mkdir dir mode = Unix.mkdir (More__fpath.to_string dir) mode in
    try
      let pmode = 0o755 in
      try Ok (mkdir dir mode; true) with
      | Unix.Unix_error (EEXIST, _, _) ->
          if is_dir (More__fpath.to_string dir) then Ok false else
      ...
      | Unix.Unix_error (ENOENT, _, _) when make_path ->
          ...
          | exception Unix.Unix_error (ENOENT, _, _) ->
              up (p :: todo) (More__fpath.parent p)
          in
          up [dir] (More__fpath.parent dir)
    with
    | Unix.Unix_error (e, _, p) ->
        if String.equal (More__fpath.to_string dir) p
        then Error (op_path_msg create_op dir (uerror e)) else
        let perr = More__fmt.str "%s: %s" p (uerror e) in
        Error (op_path_msg create_op dir perr)

  let dir_delete ~recurse dir =
    let delete_op = "Delete directory" in
    let fail e = More__fmt.failwith_notrace "%a: %s" More__fpath.pp dir e in
    let rec delete_symlink p =
      if is_symlink p then (unlink p; true) else false
    in
    let try_unlink file = match Unix.unlink (More__fpath.to_string file) with
    | () -> true
    | exception Unix.Unix_error (e, _, _) ->
        match e with
        | ENOENT -> true
    ...
    | file ->
        let file = More__fpath.(d / file) in
    ...
    let rec try_delete d todo =
      match Unix.opendir (More__fpath.to_string d) with
      | dh ->
    ...
    | [] -> ()
    | d :: ds ->
        match Unix.rmdir (More__fpath.to_string d) with
        | () -> doit ds
        | exception Unix.Unix_error (e, _, _) ->
            match e with
            ...
            | e -> fail (op_path_msg delete_op d (uerror e))
    in
    try match Unix.rmdir (More__fpath.to_string dir) with
    | () -> Ok true
    | exception Unix.Unix_error (e, _, _) ->
        match e with
        ...
        | ENOTDIR as e ->
            begin try
              if delete_symlink (More__fpath.to_string dir) then Ok true else
  ...
  let rec handle_force ~force file =
    ...
    try
      ignore (Unix.lstat (More__fpath.to_string file));
  ...
  let rec handle_force_open_fdout
    ...
    match Unix.openfile (More__fpath.to_string file) fls mode with
    | fd -> Ok fd
    | exception Unix.Unix_error (EINTR, _, _) ->
        handle_force_open_fdout ~flags ~force ~make_path ~mode file
    | exception Unix.Unix_error (ENOENT as e, _, _) when make_path ->
        begin match dir_create ~make_path (More__fpath.parent file) with
        | Ok true (* created *) ->
            handle_force_open_fdout ~flags ~force ~make_path ~mode file
        | Ok false (* existed *) ->
  ...
  let rec path_exists p =
    try (ignore (Unix.stat (More__fpath.to_string p)); Ok true) with
  ...
  let rec path_exists_stat p =
    try Ok (Some (Unix.stat (More__fpath.to_string p))) with
  ...
  let rec path_get_mode p =
    ...
    try Ok ((Unix.stat (More__fpath.to_string p)).Unix.st_perm) with
  ...
  let rec path_rename src ~force ~make_path ~dst:p =
    let err e =
      More__fmt.str "rename %a to %a: %s" More__fpath.pp src More__fpath.pp p e
    in
    ...
    | Ok () ->
        try
          Ok (Unix.rename
                (More__fpath.to_string src) (More__fpath.to_string p))
        with
        | Unix.Unix_error (ENOENT as e, _, _) when make_path ->
            begin match dir_create ~make_path (More__fpath.parent p) with
  ...
  let rec path_stat p = try Ok (Unix.stat (More__fpath.to_string p)) with
  ...
  let rec symlink ~src ~force ~make_path p =
    let err e =
      More__fmt.str "symlink %a to %a: %s" More__fpath.pp src More__fpath.pp p e
    in
    try
      Ok (Unix.symlink (More__fpath.to_string src) (More__fpath.to_string p))
    with
    | Unix.Unix_error (EEXIST, _, _) when force ->
        begin match unlink (More__fpath.to_string p) with
        | exception Unix.Unix_error (ENOENT, _, _) ->
  ...
  let rec symlink_link p =
    try
      let l = Unix.readlink (More__fpath.to_string p) in
      match More__fpath.of_string l with
      | Ok _ as v -> v
  ...
  let rec symlink_stat p = try Ok (Unix.lstat (More__fpath.to_string p)) with
...
module Tmp = struct
  let rec atomic_add set el =
    ...
    if Atomic.compare_and_set set seen (More__fpath.Set.add el seen)
    then () else atomic_add set el

  let rec atomic_remove set el =
    let seen = Atomic.get set in
    if Atomic.compare_and_set set seen (More__fpath.Set.remove el seen)
    then () else atomic_remove set el

  let delete_file file = try Fs_base.unlink (More__fpath.to_string file) with
  | Unix.Unix_error (e, _, _) -> ()

  let files = Atomic.make More__fpath.Set.empty
  ...
  let dirs = Atomic.make More__fpath.Set.empty
  let add_dir dir = atomic_add dirs dir
  let rem_dir dir = delete_dir dir; atomic_remove dirs dir

  let cleanup () =
    More__fpath.Set.iter delete_file (Atomic.get files);
    More__fpath.Set.iter delete_dir (Atomic.get dirs)
  ...
  let default_dir =
    ...
    in
    ref (More__fpath.ensure_trailing_dir_sep (More__fpath.v dir))
  ...
  let tmp_path dir name rand =
    let dir = More__fpath.to_string dir in
    match dir.[String.length dir - 1] = More__fpath.natural_dir_sep_char with
    | true -> Printf.sprintf ("%s" ^^ name) dir rand
    | false ->
        Printf.sprintf ("%s%c" ^^ name)
          dir More__fpath.natural_dir_sep_char rand

  let err dir name rand e =
    More__fmt.error "tmp file %s: %s" (tmp_path dir name rand) e
  ...
  let open'
    ...
    let dir = More__fpath.ensure_trailing_dir_sep dir in
    ...
    | n ->
        ...
        try
          ...
          let file = More__fpath.v file in
  ...
  let mkdir
    ...
    | n ->
        ...
        try
          ...
          let tdir = More__fpath.v tdir in
  ...
  let path ?(make_path = true) ?dir ?(name = format_of_string "tmp-%s") () =
    ...
    | n ->
        ...
        | exception Unix.Unix_error (ENOENT, _, _) -> Ok (More__fpath.v file)
...
module File = struct
  ...
  let rec exists file =
    match (Unix.stat (More__fpath.to_string file)).Unix.st_kind with
    | Unix.S_REG -> Ok true
    | _ -> Ok false
  ...
  let is_executable file = is_executable' (More__fpath.to_string file)
  ...
  let rec delete path =
    ...
    | exception Unix.Unix_error (e, _, _) ->
        Error (op_path_msg' op p (uerror e))
    in
    let p = More__fpath.to_string path in
  ...
  let rec truncate file size =
    try Ok (Unix.truncate (More__fpath.to_string file) size) with
  ...
  let rec link ~src ~force ~make_path file =
    let err e =
      More__fmt.error "link %a to %a: %s"
        More__fpath.pp src More__fpath.pp file e
    in
    try
      Ok (Unix.link (More__fpath.to_string src) (More__fpath.to_string file))
    with
    | Unix.Unix_error (EEXIST, _, _) when force ->
        begin match Fs_base.unlink (More__fpath.to_string file) with
        | exception Unix.Unix_error (ENOENT, _, _) ->
    ...
    | Unix.Unix_error ((ENOENT | ENOTDIR as e), _, _)
      when make_path ->
        begin match Fs_base.dir_create ~make_path (More__fpath.parent file) with
  ...
  let read_with_ic file f =
    try
      let ic =
        if More__fpath.is_dash file
        then (In_channel.set_binary_mode In_channel.stdin true; stdin)
        else open_in_bin (More__fpath.to_string file)
      in
  ...
  let read_with_fd
    ...
    try
      let fd =
        if More__fpath.is_dash file then Unix.stdin else
        Fd.openfile (More__fpath.to_string file) flags 0
      in
  ...
  let rec rename_tmp src dst =
    try
      Ok (Unix.rename (More__fpath.to_string src) (More__fpath.to_string dst))
    with
    | Unix.Unix_error (EINTR, _, _) -> rename_tmp src dst
    | Unix.Unix_error (e, _, _) ->
        Error
          (More__fmt.str "renaming %a to %a: %s"
            More__fpath.pp src More__fpath.pp dst (uerror e))
  ...
  let write_with_fd_atomic ?flags ~mode ~force ~make_path ~file f =
    ...
    in
    let dir = More__fpath.parent file in
  ...
  let write_with_fd
    ...
    =
    if More__fpath.is_dash file then Ok (f Unix.stdout) else
    if atomic
  ...
  let copy ?atomic ?mode src ~force ~make_path ~dst =
    let error e =
      More__fmt.str "copy %a to %a: %s" More__fpath.pp src More__fpath.pp dst e
    in
    ...
    try
      ...
      | None -> Some (Unix.stat (More__fpath.to_string src)).Unix.st_perm
      in
      Result.join @@ write_with_fd ?atomic ?mode ~force ~make_path dst @@
      fun fdo -> Ok (Fd.copy fdi ~dst:fdo)
    with
    | Unix.Unix_error (e, _, arg) -> More__fmt.error "%s: %s" arg (uerror e)
end
...
module Dir = struct
  ...
  let rec readdir ~dotfiles dir =
    ...
    | n when More__fpath.is_segment n -> loop ~dotfiles dir dh (n :: acc)
    | n ->
        More__fmt.failwith "%a: Invalid filename %a"
          More__fpath.pp dir More__fmt.OCaml.string n
    in
    let dh = Unix.opendir (More__fpath.to_string dir) in
  ...
  let rec exists dir =
    match (Unix.stat @@ More__fpath.to_string dir).Unix.st_kind with
    | Unix.S_DIR -> Ok true
    | _ -> Ok false
    | exception Unix.Unix_error ((ENOENT|ENOTDIR), _, _) -> Ok false
  ...
  let rec delete ~recurse dir =
    ...
    | exception Unix.Unix_error (e, _, _) ->
        Error (op_path_msg' op p (uerror e))
    in
    let p = More__fpath.to_string dir in
    match (Unix.lstat p).Unix.st_kind with
  ...
  let rec stat p = try (Unix.stat @@ More__fpath.to_string p) with
  | Unix.Unix_error (EINTR, _, _) -> stat p

  let rec lstat p = try (Unix.lstat @@ More__fpath.to_string p) with
  ...
  let fold_no_rec
    ...
    | [] -> Ok acc
    | n :: ns ->
        let full = More__fpath.(adir / n) in
        ...
        | st ->
            ...
            | Unix.S_DIR ->
                ...
                let p = if rel then More__fpath.v n else full in
        ...
        | exception Unix.Unix_error (e, _, _) ->
            stat_error (if not rel then full else More__fpath.v n) e;
            loop stat_error stat f acc adir ns
    in
    ...
    | n :: ns ->
        ...
        | st ->
            ...
            | Unix.S_DIR ->
                ...
                | None -> More__fpath.v n | Some rdir -> More__fpath.(rdir / n)
                in
  ...
  let warn_and_prune_denied ~dir ~rel _ _ p _ =
    let ap = if rel then More__fpath.(dir // p) else p in
    try (Unix.access (More__fpath.to_string ap) Unix.[R_OK; X_OK]; false) with
    | Unix.Unix_error ((EACCES | EPERM (* may happen *)), _, _) ->
        More__log.warn (fun m -> m "%a: Permission denied" More__fpath.pp p);
        true

  let warn_stat_error p e =
    More__log.warn (fun m -> m "%a: %s" More__fpath.pp p (uerror e))
  ...
  let _fold
    ...
    | Unix.Unix_error (e, _, ep) ->
        let err = uerror e in
        let err =
          if String.equal (More__fpath.to_string dir) ep then err else
          More__fmt.str "%s: %s" ep err
        in
  ...
  | Unix.S_DIR -> More__fpath.ensure_trailing_dir_sep f :: acc
  | _ -> f :: acc
  ...
  let copy
    ...
    | Unix.S_REG ->
        ...
        let mode = st.Unix.st_perm in
        let src = More__fpath.(src // p) in
        let dst = More__fpath.(dst // p) in
        (cp ~mode src dst |> More__result.error_to_failure); acc
    | Unix.S_LNK ->
        if prune st name p () then acc else
        let dst = More__fpath.(dst // p) in
        let src = More__fpath.(src // p) in
    ...
    | _ ->
        More__fmt.failwith "%a: Not a regular file, directory or symlink"
          More__fpath.pp More__fpath.(src // p)
    in
    let rec chmod_dirs = function
    | [] -> ()
    | (d, m) :: ds ->
        (Fs_base.path_set_mode d m) |> More__result.error_to_failure;
        chmod_dirs ds
    in
    ...
    let* tdst =
      if atomic
      then Tmp.mkdir ~make_path ~dir:(More__fpath.parent dst) ()
      else Result.bind (Fs_base.dir_create ~make_path dst) (fun _ -> Ok dst)
    in
    try
      let src_mode =
        Fs_base.path_get_mode src |> More__result.error_to_failure
      in
      let chmods =
  ...
  let set_default_tmp p =
    Tmp.default_dir := More__fpath.ensure_trailing_dir_sep p
  ...
  let rec cwd () =
    let err e = More__fmt.error "get cwd: %s" e in
    match More__fpath.of_string (Unix.getcwd ()) with
    | Ok dir when More__fpath.is_absolute dir -> Ok dir
    | Ok dir -> err (More__fmt.str "%a is relative" More__fpath.pp dir)
    | Error e -> err e
    | exception Unix.Unix_error (EINTR, _, _) -> cwd ()
    | exception Unix.Unix_error (e, _, _) -> err (uerror e)

  let rec set_cwd dir =
    let err e = More__fmt.error "set cwd to %a: %s" More__fpath.pp dir e in
    try Ok (Unix.chdir (More__fpath.to_string dir)) with
  ...
  let err_dir dir fmt = More__fmt.error ("%s directory: " ^^ fmt) dir
  let fpath_of_env_var dir var = match Env.var ~empty_is_none:true var with
  | None -> None
  | Some p ->
      match More__fpath.of_string p with
  ...
  let user () =
    ...
    match
      ...
      | Some id ->
          ...
          | Some id -> id
      in
      More__fpath.of_string (Unix.getpwuid uid).Unix.pw_dir
    with
    | Ok _ as v -> v
  ...
  let data_fallback () = home_fallback data_dir (More__fpath.v ".local/share")
  let data () =
  ...
  let cache_fallback () = home_fallback cache_dir (More__fpath.v ".cache")
  let cache () =
  ...
  let state_fallback () = home_fallback state_dir (More__fpath.v ".local/state")
  let state () =
...
module Path = struct
  ...
  let rec must_exist p =
    try Ok (ignore (Unix.stat (More__fpath.to_string p))) with
  ...
  let rec realpath p =
    try More__fpath.of_string (Unix.realpath (More__fpath.to_string p)) with
  ...
  let rec exists_realpath p =
    try
      ...
      More__fpath.of_string (Unix.realpath (More__fpath.to_string p))
    with
  ...
  let copy
    ...
    in
    let* src_stat =
      if More__fpath.is_dash src then Ok None else
      match Fs_base.path_stat src with
      | Ok stat -> Ok (Some stat)
      | Error e -> Error (err e)
    in
    let* dst =
      if More__fpath.is_dash dst then Ok dst else
      match Fs_base.path_exists_stat dst with
      | Error e -> Error (err e)
      | Ok Some { st_kind = S_DIR } -> Ok More__fpath.(dst / basename src)
      | Ok _ -> Ok dst
    in
    match src_stat with
    | Some { st_kind = S_DIR } ->
  ...
  let is_mount_point p =
    let err e = More__fmt.str "is_mount_point: %s" e in
    match Fs_base.path_stat p with
    | Error e -> Error (err e)
    | Ok stat ->
        match Fs_base.path_stat More__fpath.(p / "..") with
...
module Cmd = struct
  ...
  let tool_is_path t = String.exists More__fpath.is_dir_sep_char t
  let tool_file ~dir tool = match dir.[String.length dir - 1] with
  | c when More__fpath.is_dir_sep_char c -> dir ^ tool
  | _ -> String.concat More__fpath.natural_dir_sep [dir; tool]
  ...
  | p ->
      ...
      | "" ->
          Error (`Dirs (String.split_on_char More__fpath.search_path_sep.[0] p))
      | p ->
          let dir, p =
            let sep = More__fpath.search_path_sep in
            match More__string.split_first ~sep p with
            | None -> p, ""
            | Some (dir, p) -> dir, p
          in
          ...
          | false -> loop tool p
          | true -> Ok (More__fpath.v tool_file)
      in
      loop tool p

  let search_in_dirs ~dirs tool =
    let rec loop tool = function
    | [] -> Error (`Dirs (List.map More__fpath.to_string dirs))
    | d :: dirs ->
        let tool_file = tool_file ~dir:(More__fpath.to_string d) tool in
        match File.is_executable' tool_file with
  ...
  let path_search ?(win_exe = Sys.win32) ?path () = fun cmd ->
    match More__cmd.find_tool cmd with
    | None -> More__fmt.error "No tool to lookup: the command is empty"
    | Some tool ->
        let tool =
          let tool = More__fpath.to_string tool and suffix = ".exe" in
        ...
        | true ->
            ...
            then Ok (More__cmd.set_tool (More__fpath.v tool) cmd)
            else More__fmt.error "%s: No such executable file" tool
        | false ->
            ...
            in
            match file with
            | Ok file -> Ok (More__cmd.set_tool file cmd)
            | Error (`Msg e) -> More__fmt.error "%s: %s" tool e
            | Error (`Dirs dirs) ->
                More__fmt.error "@[<v1>%s: No such tool found in:@,%a@]"
                  tool (More__fmt.(list string)) dirs
  ...
  let rec get_first ?(search = default_search) cmds =
    ...
    | Error e ->
        ...
        | [] ->
            More__fmt.error "@[<v1>%a@,neither as:@,%a@]"
              More__fmt.lines e (More__fmt.list More__cmd.pp) (List.tl cmds)
        | cmd :: cmds ->
  ...
  | `Exited n -> More__fmt.pf ppf "@[exited [%d]@]" n
  | `Signaled s -> More__fmt.pf ppf "@[signaled [%a]@]" More__fmt.sys_signal s

  let pp_cmd_status ppf (cmd, st) =
    More__fmt.pf ppf "cmd [%s]: %a" (More__cmd.to_string cmd) pp_status st

  (* Process standard inputs *)

  type stdi =
  | In_string of string
  | In_file of More__fpath.t
  ...
  let in_null = In_file More__fpath.null
  ...
  | In_string s ->
      begin try
        ...
        let f, fd = More__result.error_to_failure (Tmp.open' ~flags ()) in
        Fd.Set.add fd fds;
      ...
      with
      | Unix.Unix_error (e, _, _) ->
          More__fmt.failwith_notrace "tmp file for stdin: %s" (uerror e)
      end
  | In_file f ->
      try
        let f = More__fpath.to_string f in
        let fd = Fd.openfile f Unix.[O_RDONLY] 0o644 in
        Fd.Set.add fd fds; fd
      with Unix.Unix_error (e, _, _) ->
        More__fmt.failwith_notrace "open file %a for stdin: %s"
          More__fpath.pp f (uerror e)
  ...
  | Out_file of
      { mode : int; force : bool; make_path : bool; file : More__fpath.t }
  ...
  let out_null = out_file ~force:true ~make_path:false More__fpath.null
  ...
  | Out_file { mode; force; make_path; file } ->
      ...
      match
        Fs_base.handle_force_open_fdout ~flags ~force ~make_path ~mode file
      with
      | Error e -> More__fmt.failwith_notrace "open for output: %s" e
      | Ok fd -> Fd.Set.add fd fds; fd

  (* Low-level command spawn *)

  type pid = { pid : int; cmd : More__cmd.t }
  type spawn_tracer =
    pid option -> Env.assignments option -> cwd:More__fpath.t option ->
    More__cmd.t -> unit

  let spawn_tracer_nop _ _ ~cwd:_ _ = ()
  let spawn_tracer_log level =
    if level = More__log.Quiet then spawn_tracer_nop else
    let header = function
    ...
    | None -> ()
    | Some env ->
        More__fmt.pf ppf "%a@," (More__fmt.list More__fmt.OCaml.string) env
    in
    fun pid env ~cwd cmd ->
      More__log.msg level (fun m ->
          m ~header:(header pid) "@[<v>%a%a@]" pp_env env More__cmd.pp_dump cmd)
  ...
  | Unix.Unix_error (e, _, _) ->
      More__fmt.failwith_notrace "getcwd: %s" (uerror e)

  let rec chdir cwd = try Unix.chdir cwd with
  | Unix.Unix_error (EINTR, _, _) -> chdir cwd
  | Unix.Unix_error (e, _, _) ->
      More__fmt.failwith_notrace "chdir %s: %s" cwd (uerror e)

  let spawn_err cmd e =
    if More__cmd.is_empty cmd then Error e else
    More__fmt.error "cmd %s: %s" (More__cmd.to_string cmd) e

  let spawn_cwd = function None -> getcwd () | Some d -> More__fpath.to_string d
  let spawn_env = function
  | None -> Unix.environment ()
  | Some e -> Array.of_list e

  let _spawn fds ?env ?cwd ~stdin ~stdout ~stderr cmd =
    match More__cmd.to_list cmd with
  ...
  | exception Unix.Unix_error (e, _, _) ->
      More__fmt.failwith_notrace "waitpid [%d]: %s" pid.pid (uerror e)
  ...
  let run ?env ?cwd ?stdin ?stdout ?stderr cmd =
    ...
    | Ok (`Exited 0) -> Ok ()
    | Ok st -> More__fmt.error "%a" pp_cmd_status (cmd, st)
    | Error _ as e -> e
  ...
  let run_out ?env ?cwd ?stdin ?stderr ~trim cmd =
    ...
    | Ok (st, "") -> More__fmt.error "%a" pp_cmd_status (cmd, st)
    | Ok (st, o) ->
        More__fmt.error "@[<v>%a after outputing:@, @[%a@]@]"
          pp_cmd_status (cmd, st) More__fmt.lines o
  ...
  | exception Unix.Unix_error (e, _, _) ->
      More__fmt.error "poll_status: waitpid [%d]: %s" pid.pid (uerror e)
  ...
  | exception Unix.Unix_error (e, _, _) ->
      More__fmt.error "waitpid [%d]: %s" pid.pid (uerror e)

  let spawn_wait pid = match spawn_wait_status pid with
  | Ok (`Exited 0) -> Ok ()
  | Ok st ->
      More__fmt.error "@[waitpid [%d]: %a@]" pid.pid pp_cmd_status (pid.cmd, st)
  | Error _ as e -> e

  let kill pid sg = match Unix.kill pid.pid sg with
  | () -> Ok ()
  | exception Unix.Unix_error (e, _, _) ->
      More__fmt.error "kill %d with %a: %s" pid.pid More__fmt.sys_signal sg
        (uerror e)
  ...
  let execv ?env ?cwd ?argv0 cmd =
    let err_execv f e = More__fmt.error "execv %s: %s" f e in
    match More__cmd.to_list cmd with
    ...
    | file :: args as all ->
        ...
        try
          ...
          | Some cwd ->
            let old_cwd = getcwd () in
            chdir (More__fpath.to_string cwd);
...
module Exit = struct
  let log_error e = More__log.err (fun m -> m "@[%a@]" More__fmt.lines e)

  type code = int
  type execv =
    { env : Env.assignments option;
      cwd : More__fpath.t option;
      argv0 : string option;
      cmd : More__cmd.t }
...
module Pty = struct
  ...
  | Unix.Unix_error (e, name, _) ->
      Error (More__fmt.str "%s: %s" name (uerror e))
  ...
  let set_echo pty c_echo =
    ...
    with
    | Unix.Unix_error (e, name, _) ->
        Error (More__fmt.str "pty set echo: %s: %s" name (uerror e))
  ...
  let open_with_spawn ?env ?cwd ?stdin ?stdout ?stderr cmd =
    ...
    with
    | Unix.Unix_error (e, name, _) ->
        Error (More__fmt.str "pty open %s: %s" name (uerror e))
  ...
  let read_in_buf ?timeout ?max pty =
    ...
    | None -> -1.0 | Some dur -> More__mtime.Span.to_float_s dur
    in
    let ready, _, _ =
  ...
  let read ?timeout ?max pty =
    ...
    with
    | Unix.Unix_error (e, name, _) ->
        Error (More__fmt.str "pty read: %s: %s" name (uerror e))
  ...
  let seek ?(log = false (* FIXME removed that *)) ?timeout pty mark =
    ...
    try
      let rec loop prev ?timeout pty mark =
        ...
        then Error (More__fmt.str "pty seek: timeout or EOF before mark") else
        let chunk = buf_to_string pty in
        let search = prev ^ chunk (* XXX quick hack can be improved *) in
        match More__string.find_first ~sub:mark search with
        | None ->
    ...
    with
    | Unix.Unix_error (e, name, _) ->
        Error (More__fmt.str "pty seek: %s: %s" name (uerror e))
  ...
  let interact pty =
    ...
    with
    | Unix.Unix_error (e, name, _) ->
        Error (More__fmt.str "pty interact: %s: %s" name (uerror e))
end
...
module Cpu = struct
  external logical_count : unit -> int = "ocaml_more_cpu_logical_count"
  ...
  module Time = struct
    ...
    module Span = struct
      ...
      let make ~utime ~stime ~children_utime ~children_stime =
        { utime = More__mtime.Span.to_uint64_ns utime;
          stime = More__mtime.Span.to_uint64_ns stime;
          children_utime = More__mtime.Span.to_uint64_ns children_utime;
          children_stime = More__mtime.Span.to_uint64_ns children_stime;  }

      let zero =
        { utime = 0L; stime = 0L; children_utime = 0L; children_stime = 0L }

      let utime c = More__mtime.Span.of_uint64_ns c.utime
      let stime c = More__mtime.Span.of_uint64_ns c.stime
      let children_utime c = More__mtime.Span.of_uint64_ns c.children_utime
      let children_stime c = More__mtime.Span.of_uint64_ns c.children_stime
    end
...
external uname_machine : unit -> string = "ocaml_more_uname_machine"
external uname_sysname : unit -> string = "ocaml_more_uname_sysname"
...
module Name = struct
  ...
  let of_string ?family s =
    let id = More__string.Ascii.lowercase s in
  ...
  let pp ppf a = More__fmt.string ppf @@ match a with
  | Bsd _ -> "bsd" | Darwin _ -> "darwin"
  | Linux _ -> "linux" | Windows _ -> "windows"
  | Other id -> id

  let pp_id = More__fmt.using id More__fmt.string
  ...
  let darwin_info uname_sysname =
    let get_plist_key key xml = (* Easy to thwart… *)
      let find ~start ~sub s = match More__string.find_first ~start ~sub s with
  ...
  let etc_os_release_info uname_sysname =
    ...
    match File.read (More__fpath.v info) with
    | Error e ->
        More__log.debug (fun m -> m "%s" e);
    ...
    | Ok data ->
        ...
        | [] ->
            ...
            id, id_like, version
        | l :: ls ->
            match More__string.split_first ~sep:"=" (String.trim l) with
  ...
  let init_info () =
    let sysname = More__string.Ascii.lowercase (uname_sysname ()) in
...
module Arch = struct
  ...
  let pp_id = More__fmt.using id More__fmt.string
  let pp_bits =
    More__fmt.using bits More__fmt.(option int ~none:(any "<unknown>"))
end
