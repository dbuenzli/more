...
module Search = struct
  ...
  (* The following searches from the end of string. Except for changes
     marked with explicit comments this a cut and paste of the code to
     search forward, except the [get] function it invokes map indices
     from the range [0;n-1] to [n-1;0] which we call the reverse space *)

  let[@inline] get s i = get s (String.length s - 1 - i)
  ...
  let rfind ~start ~sub ~rsub_lp:(l, p) ~rsub_periodic s = (* -1 on not found *)
    ...
    if not (0 <= start && start <= slen) then invalid_start ~start slen else
    let start =
      (* In the reverse space we find the index of the end of [sub] so we
         need to allow to adjust start to search from there which is
         the index [start + sublen - 1]. This index then converted into the
    ...
    with Exit ->
      (* This transforms back from the reverse space and compensates
         for the fact that we found the location of the end of [sub]. *)
      slen - 1 - (!j + (sublen - 1))
end
...
let fold_ascii_lines ~drop_newlines:strip f acc s =
...
let is_sep_default = More__char.Ascii.is_white
let is_token_default = More__char.Ascii.is_graphic

let take_token ?(is_sep = is_sep_default) ?(is_token = is_token_default) s =
  let len = length s in
  let i = ref 0 in
  ...
  if !i = first then "" else
  subrange ~first ~last:(!i - 1) s

let drop_token ?(is_sep = is_sep_default) ?(is_token = is_token_default) s =
  let len = length s in
  let i = ref 0 in
  while !i < len && is_sep s.[!i] do incr i done;
  if !i >= len then s else
...
let cut_token ?(is_sep = is_sep_default) ?(is_token = is_token_default) s =
  let len = length s in
  let i = ref 0 in
  while !i < len && is_sep s.[!i] do incr i done;
  if !i >= len then "", s else
...
let tokens ?(is_sep = More__char.Ascii.is_white) s =
...
module Ascii = struct
  ...
  let is_valid s =
    ...
    | true -> true
    | false when unsafe_get s i > More__char.Ascii.max -> false
    | false -> loop max (i + 1)
    in
    loop (length s - 1) 0
  ...
  let uppercase s =
    caseify More__char.Ascii.is_lower More__char.Ascii.uppercase s

  let lowercase s =
    caseify More__char.Ascii.is_upper More__char.Ascii.lowercase s
  ...
  let capitalize s =
    caseify_first More__char.Ascii.is_lower More__char.Ascii.uppercase s

  let uncapitalize s =
    caseify_first More__char.Ascii.is_upper More__char.Ascii.lowercase s
  ...
  let to_hex s =
    ...
    | false ->
        ...
        Bytes.set h (k    ) (More__char.Ascii.lower_hex_digit_of_int (c lsr 4));
        Bytes.set h (k + 1) (More__char.Ascii.lower_hex_digit_of_int (c      ));
  ...
  | Error i ->
      ...
      | false -> More__fmt.error "Byte %d: not an ASCII hexadecimal digit" i
  ...
  let set_ascii_unicode_escape b k c = (* for c <= 0x7F *)
    ...
    Bytes.set b (k + 5) (More__char.Ascii.upper_hex_digit_of_int (byte lsr 4));
    Bytes.set b (k + 6) (More__char.Ascii.upper_hex_digit_of_int (byte      ));
    Bytes.set b (k + 7) '}';
    k + 8
  ...
  let set_hex_escape b k c =
    ...
    Bytes.set b (k + 2) (More__char.Ascii.upper_hex_digit_of_int (byte lsr 4));
    Bytes.set b (k + 3) (More__char.Ascii.upper_hex_digit_of_int (byte      ));
    k + 4
  ...
  let unescape =
    ...
    | false ->
          ...
          let j = i + 3 in
          if j > max then raise_notrace (Illegal_escape i) else
          if More__char.Ascii.is_hex_digit s.[i + 2] &&
             More__char.Ascii.is_hex_digit s.[i + 3]
          then 4
    ...
    | false ->
        (* assert (s.[i+1] = 'x') *)
        let hi = More__char.Ascii.hex_digit_to_int s.[i + 2] in
        let lo = More__char.Ascii.hex_digit_to_int s.[i + 3] in
  ...
  let ocaml_unescape =
    let char_len_at s i =
      ...
      | c when More__char.Ascii.is_digit c ->
          let j = i + 3 in
          if j > max then raise_notrace (Illegal_escape i) else
          if More__char.Ascii.is_digit s.[i + 2] &&
             More__char.Ascii.is_digit s.[i + 3]
          then 4
          else raise_notrace (Illegal_escape i)
      | _ -> raise_notrace (Illegal_escape i)
    in
    let set_char b k s i =
      ...
      | '"' -> Bytes.set b k '"'; i + 2
      | 'o' ->
          let o3 = More__char.Ascii.hex_digit_to_int s.[i + 2] in
          let o2 = More__char.Ascii.hex_digit_to_int s.[i + 3] in
          let o1 = More__char.Ascii.hex_digit_to_int s.[i + 4] in
          let byte = o3 * 64 + o2 * 8 + o1 in
          if byte > 255 then raise_notrace (Illegal_escape i) else
          Bytes.set b k (Char.chr byte); i + 5
      | c when More__char.Ascii.is_digit c ->
          let d3 = More__char.Ascii.hex_digit_to_int s.[i + 1] in
          let d2 = More__char.Ascii.hex_digit_to_int s.[i + 2] in
          let d1 = More__char.Ascii.hex_digit_to_int s.[i + 3] in
...
let pp = More__fmt.string

(* Sets and maps *)

module Set = struct
  include Set.Make (String)
  let pp ?sep pp_elt = More__fmt.iter ?sep iter pp_elt
  let pp_dump ppf ss =
    More__fmt.pf ppf "@[<1>{%a}@]"
      (pp ~sep:More__fmt.sp More__fmt.OCaml.string) ss
end
...
module Map = struct
  ...
  let get_or_hint ?(pp_key = More__fmt.code) ~kind k m =
    match get_or_suggest k m with
    | Ok _ as v -> v
    | Error suggs ->
        let kind ppf () = More__fmt.string ppf kind in
        let hint = More__fmt.did_you_mean in
        let pp = More__fmt.unknown' ~kind pp_key ~hint in
        More__fmt.error "@[%a@]" pp (k, suggs)

  let pp ?sep pp_binding = More__fmt.iter_bindings ?sep iter pp_binding
  let pp_dump pp_v ppf m =
    let pp_binding ppf (k, v) =
      More__fmt.pf ppf "@[<1>(@[%a@],@ @[%a@])@]"
        More__fmt.OCaml.string k pp_v v
    in
    More__fmt.pf ppf "@[<1>{%a}@]" (pp ~sep:More__fmt.sp pp_binding) m

  let pp_dump_string_map ppf m =
    pp_dump More__fmt.OCaml.string ppf m
end
...
let unique ?(limit = 1_000_000) ~exists n =
  let rec loop i n = match i > limit with
  | true ->
      More__fmt.invalid_arg "Could not make %s unique after %d retries." n limit
  | false ->
      let r = More__fmt.str "%s~%d" n i in
