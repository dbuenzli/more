...
#if defined(OCAML_MORE_DARWIN) || defined(OCAML_MORE_POSIX) /* inc. Cygwin */

#include <sys/utsname.h>

CAMLprim value ocaml_more_uname_machine (value unit)
{
  struct utsname u;
  int rc = uname (&u);
  if (rc < 0)
...
CAMLprim value ocaml_more_uname_sysname (value unit)
{
  struct utsname u;
  int rc = uname (&u);
  if (rc < 0)
...
#if defined(OCAML_MORE_WINDOWS) || defined(OCAML_MORE_CYGWIN)
...
CAMLprim value ocaml_more_caml_win32_version (value unit)
{
