...
(** Networking URLs, endpoints, connections and listeners.
...
module Endpoint : sig
  ...
  (** {1:wait_connectable Wait connectable}

      {b TODO.} Review with actions in mind. *)

  val wait_connectable' :
    ?socket_type:Unix.socket_type -> timeout:More__mtime.Span.t -> t ->
    ([`Ready | `Timeout], string) result
  (** [wait_connectable' ~timeout ep st] blocks until [ep] becomes
...
module Connection : sig
  ...
  (** [open ~peer ()] opens a connection to a peer listening on [peer] with:
      {ul
      {- [nonblock] if [true] the socket is in non-blocking mode (defaults
         to [false]).}
...
module Listener : sig
  ...
  (** [accept l] offers a connection on [l]. Blocks until a client
      ...
      be closed with {!Connection.close_noerr} and its underlying file
      desctriptor has [cloexec] set to [true]. *)
