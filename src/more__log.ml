...
| "debug" ->  Ok Debug
| e ->
    let pp_level = More__fmt.code in
    let kind = More__fmt.any "log level" in
    let dom =
      ["quiet"; "stdout"; "stderr"; "error"; "warning"; "info"; "debug"]
    in
    let unknown = More__fmt.(unknown' ~kind pp_level ~hint:must_be) in
    More__fmt.error "%a" unknown (e, dom)
...
| Stdout -> More__fmt.st header_stdout_style ppf header
| Stderr -> More__fmt.st header_stderr_style ppf header
| Error -> More__fmt.st header_err_style ppf header
| Warning -> More__fmt.st header_warn_style ppf header
| Info -> More__fmt.st header_info_style ppf header
| Debug -> More__fmt.st header_debug_style ppf header
| Quiet -> ()

let pp_level ppf level = match level with
| Stdout | Stderr -> ()
| Error -> More__fmt.st error_style ppf "Error"
| Warning -> More__fmt.st warning_style ppf "Warning"
| Info -> More__fmt.st info_style ppf "Info"
| Debug -> More__fmt.st debug_style ppf "Debug"
| Quiet -> ()
...
let default_kmsg k level msgf =
  msgf @@ fun ?header fmt ->
  let ppf = if level = Stdout then More__fmt.stdout else More__fmt.stderr in
  let finish ppf =
    Format.pp_close_box ppf (); Format.pp_close_box ppf ();
    More__fmt.flush_nl ppf ();
    k ()
  in
  ...
  | None ->
      ...
      | level ->
          (* "%s: @[%a: " *)
          More__fmt.string ppf exec;
          More__fmt.string ppf ": ";
          Format.pp_open_box ppf 0;
          pp_level ppf level;
          More__fmt.string ppf ": ";
      end
