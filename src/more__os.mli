...
module Path : sig
  ...
  val exists : More__filepath.t -> (bool, string) result
  ...
  val must_exist : More__filepath.t -> (unit, string) result
  ...
  val delete : recurse:bool -> More__filepath.t -> (bool, string) result
  ...
  val rename :
    More__filepath.t -> force:bool -> make_path:bool -> dst:More__filepath.t ->
    (unit, string) result
  ...
  val realpath : More__filepath.t -> (More__filepath.t, string) result
  ...
  val exists_realpath :
    More__filepath.t -> (More__filepath.t option, string) result
  ...
  val copy :
    ...
    ?stat_error:(More__filepath.t -> Unix.error -> unit) ->
    ?prune:(Unix.stats -> string -> More__filepath.t -> bool) ->
    follow_symlinks:bool -> recurse:bool -> More__filepath.t ->
    force:bool -> make_path:bool ->
    dst:More__filepath.t -> (unit, string) result
  ...
  val get_mode : More__filepath.t -> (int, string) result
  (** [get_mode p] is the file mode of [p]. Symbolic links are followed. *)

  val set_mode : More__filepath.t -> int -> (unit, string) result
  (** [set_mode file p] sets the file mode of [file] to [p]. Symbolic
      links are followed. *)

  val stat : More__filepath.t -> (Unix.stats, string) result
  ...
  val exists_stat : More__filepath.t -> (Unix.stats option, string) result
  ...
  val is_mount_point : More__filepath.t -> (bool, string) result
  ...
  val symlink :
    src:More__filepath.t -> force:bool -> make_path:bool -> More__filepath.t ->
    (unit, string) result
  ...
  val symlink_link : More__filepath.t -> (More__filepath.t, string) result
  (** [symlink_link p] is [Ok l] if [p] is a symbolic link to [l]. *)

  val symlink_stat : More__filepath.t -> (Unix.stats, string) result
  ...
  (** {1:tmppaths Temporary paths}
      ...
      See also temporary {{!More.Os.File.tmpfiles}files} and
      {{!More.Os.Dir.tmpdirs}directories}. *)
  ...
  val tmp :
    ?make_path:bool -> ?dir:More__filepath.t -> ?name:tmp_name -> unit ->
    (More__filepath.t, string) result
  (** [tmp ~make_path ~dir ~name ()] is a file system path in [dir]
      ...
      {- [dir] is the directory in which the temporary file is created.
         It defaults to {!More.Os.Dir.default_tmp}[ ()].}
...
module File : sig
  ...
  val is_executable : More__filepath.t -> bool
  ...
  val delete : More__filepath.t -> (bool, string) result
  ...
  val truncate : More__filepath.t -> int -> (unit, string) result
  (** [truncate file size] truncates [file] to [size]. *)

  (** {1:reads Reading} *)

  val read : More__filepath.t -> (string, string) result
  ...
  val read_with_fd :
    ?flags:Unix.open_flag list -> More__filepath.t ->
  ...
  val write :
    ...
    More__filepath.t -> string -> (unit, string) result
  ...
  val write_with_fd :
    ...
    make_path:bool -> More__filepath.t -> (Unix.file_descr -> ('a, 'b) result) ->
  ...
  val write_with_oc :
    ?flags:Unix.open_flag list -> ?atomic:bool -> ?mode:int -> force:bool ->
    make_path:bool -> More__filepath.t -> (out_channel -> ('a, 'b) result) ->
    (('a, 'b) result, string) result
  ...
  val copy :
    ?atomic:bool -> ?mode:int -> More__filepath.t ->
    force:bool -> make_path:bool -> dst:More__filepath.t -> (unit, string) result
  ...
  (** {1:tmpfiles Temporary files}
      ...
      See also temporary {{!More.Os.Dir.tmpdirs}directories} and
      {{!More.Os.Path.tmppaths}paths}. *)

  val with_tmp :
    ?flags:Unix.open_flag list -> ?mode:int -> ?make_path:bool ->
    ?dir:More__filepath.t -> ?name:Path.tmp_name -> (More__filepath.t -> 'a) ->
    ('a, string) result
  ...
  val with_tmp_fd :
    ?flags:Unix.open_flag list -> ?mode:int -> ?make_path:bool ->
    ?dir:More__filepath.t -> ?name:Path.tmp_name ->
    (More__filepath.t -> Unix.file_descr -> 'a) -> ('a, string) result
  ...
  val with_tmp_oc :
    ?flags:Unix.open_flag list -> ?mode:int -> ?make_path:bool ->
    ?dir:More__filepath.t -> ?name:Path.tmp_name ->
    (More__filepath.t -> out_channel -> 'a) -> ('a, string) result
  (** [with_tmp_oc] is like {!with_tmp_fd} but uses an OCaml output channel
      instead of a file decriptor. *)

  val open_tmp_fd :
    ?flags:Unix.open_flag list -> ?mode:int -> ?make_path:bool ->
    ?dir:More__filepath.t -> ?name:Path.tmp_name -> unit ->
    (More__filepath.t * Unix.file_descr, string) result
...
module Dir : sig
  ...
  val create :
    ?mode:int -> make_path:bool -> More__filepath.t -> (bool, string) result
  ...
  val fold :
    ?rel:bool ->
    ?stat_error:(More__filepath.t -> Unix.error -> unit) ->
    ?prune_dir:(Unix.stats -> string -> More__filepath.t -> 'a -> bool) ->
    dotfiles:bool -> follow_symlinks:bool -> recurse:bool ->
    (Unix.stats -> string -> More__filepath.t -> 'a -> 'a) ->
    More__filepath.t -> 'a -> ('a, string) result
  ...
  val fold_files :
    ...
    recurse:bool -> (Unix.stats -> string -> More__filepath.t -> 'a -> 'a) ->
    More__filepath.t -> 'a -> ('a, string) result
  (** [fold_files] is like {!fold} but [f] is only applied to
      non-directory files. *)

  val fold_dirs :
    ?rel:bool -> ?stat_error:(More__filepath.t -> Unix.error -> unit) ->
    ?prune_dir:(Unix.stats -> string -> More__filepath.t -> 'a -> bool) ->
    dotfiles:bool -> follow_symlinks:bool ->
    recurse:bool -> (Unix.stats -> string -> More__filepath.t -> 'a -> 'a) ->
    More__filepath.t -> 'a -> ('a, string) result
  ...
  val contents :
    ...
    ?rel:bool ->
    ?stat_error:(More__filepath.t -> Unix.error -> unit) ->
    ?prune_dir:(Unix.stats -> string -> More__filepath.t ->
                More__filepath.t list -> bool) ->
    dotfiles:bool -> follow_symlinks:bool ->
    recurse:bool -> More__filepath.t -> (More__filepath.t list, string) result
  ...
  val path_list :
    Unix.stats -> string -> More__filepath.t -> More__filepath.t list ->
    More__filepath.t list
  ...
  val warn_and_prune_denied :
    dir:More__filepath.t -> rel:bool ->
    (Unix.stats -> string -> More__filepath.t -> 'a -> bool)
  ...
  val warn_stat_error : More__filepath.t -> Unix.error -> unit
  ...
  val copy :
    ...
    ?stat_error:(More__filepath.t -> Unix.error -> unit) ->
    ?prune:(Unix.stats -> string -> More__filepath.t -> bool) ->
    follow_symlinks:bool -> make_path:bool -> More__filepath.t ->
    dst:More__filepath.t -> (unit, string) result
  ...
  val cwd : unit -> (More__filepath.t, string) result
  (** [cwd ()] is the current working directory. The resulting path
      is guaranteed to be absolute. *)

  val set_cwd : More__filepath.t -> (unit, string) result
  (** [set_cwd dir] sets the current working directory to [dir]. *)

  val with_cwd : More__filepath.t -> (unit -> 'a) -> ('a, string) result
  ...
  val default_tmp : unit -> More__filepath.t
  ...
  val set_default_tmp : More__filepath.t -> unit
  (** [set_default_tmp p] sets the value returned by {!default_tmp} to
      [p]. *)

  (** {1:tmpdirs Temporary directories}

      See also temporary {{!More.Os.File.tmpfiles}files}
      or {{!More.Os.Path.tmppaths}paths}. *)

  val with_tmp :
    ?mode:int -> ?make_path:bool -> ?dir:More__filepath.t ->
    ?name:Path.tmp_name -> (More__filepath.t -> 'a) -> ('a, string) result
  (** [with_tmp ~mode ~make_path ~dir ~name f] creates a temporary empty
      ...
      {- [name] is used to construct the filename of the directory,
         see {!type:More.Os.Path.tmp_name} for details. It defaults to
         ["tmp-%s"].}
      {- [dir] is the directory in which the temporary file is created.
         It defaults to {!More.Os.Dir.default_tmp}.}
      {- If [make_path] is [true] (default) and [dir] doesn't exist the
  ...
  val tmp :
    ?mode:int -> ?make_path:bool -> ?dir:More__filepath.t ->
    ?name:Path.tmp_name -> unit -> (More__filepath.t, string) result
  ...
  val user : unit -> (More__filepath.t, string) result
  ...
  val config : unit -> (More__filepath.t, string) result
  ...
  val data : unit -> (More__filepath.t, string) result
  ...
  val cache : unit -> (More__filepath.t, string) result
  ...
  val runtime : unit -> (More__filepath.t, string) result
  ...
  val state : unit -> (More__filepath.t, string) result
...
module Env : sig
  ...
  (** [of_assignments ~init ss] folds over strings in [ss],
      {{!More.String.split_first}splits} them at the leftmost ['=']
  ...
  val pp_assignments : assignments More__fmt.t
...
module Cmd : sig
  ...
  val path_search :
    ?win_exe:bool -> ?path:More__filepath.t list -> unit -> More__cmd.tool_search
  (** [path_search ~win_exe ~path () cmd] searches the
      {{!More.Cmd.type-tool}tool} of [cmd] in the [path]
  ...
  val find : ?search:More__cmd.tool_search -> More__cmd.t -> More__cmd.t option
  (** [find ~search cmd] is [cmd] with its {!More.Cmd.val-tool}
  ...
  val find_first :
    ?search:More__cmd.tool_search -> More__cmd.t list -> More__cmd.t option
  (** [find_first ?search cmds] is [List.find_map (find ?search) cmds]. *)

  val get : ?search:More__cmd.tool_search -> More__cmd.t ->
    (More__cmd.t, string) result
  (** [get] is like {!find} except but return an error message if [Ok None]
      is returned. *)

  val get_first :
    ?search:More__cmd.tool_search -> More__cmd.t list ->
    (More__cmd.t, string) result
  ...
  val pp_status : status More__fmt.t
  ...
  val pp_cmd_status : (More__cmd.t * status) More__fmt.t
  ...
  val in_file : More__filepath.t -> stdi
  ...
  val out_file :
    ?mode:int -> force:bool -> make_path:bool -> More__filepath.t -> stdo
  ...
  val run_status :
    ?env:Env.assignments -> ?cwd:More__filepath.t -> ?stdin:stdi ->
    ?stdout:stdo -> ?stderr:stdo -> More__cmd.t -> (status, string) result
  ...
  val run_status_out :
    ?env:Env.assignments -> ?cwd:More__filepath.t -> ?stdin:stdi ->
    ?stderr:[`Stdo of stdo | `Out] -> trim:bool -> More__cmd.t ->
  ...
  val run :
    ?env:Env.assignments -> ?cwd:More__filepath.t -> ?stdin:stdi ->
    ?stdout:stdo -> ?stderr:stdo -> More__cmd.t -> (unit, string) result
  ...
  val spawn :
    ?env:Env.assignments -> ?cwd:More__filepath.t -> ?stdin:stdi -> ?stdout:stdo ->
    ?stderr:stdo -> More__cmd.t -> (pid, string) result
  ...
  val spawn_tracer_log : More__log.level -> spawn_tracer
  ...
  val execv :
    ?env:Env.assignments -> ?cwd:More__filepath.t -> ?argv0:string ->
    More__cmd.t -> ('a, string) result
  (** [execv ~env ~cwd cmd] executes the realpath pointed by
      {!More.Cmd.val-tool}[ cmd] as a new process in environment with [cmd] as
      the {!Sys.argv} of this process. The function only returns in
      case of error. [env] defaults to
      {!More.Os.Env.current_assignments}[ ()], [cwd] to
      {!More.Os.Dir.val-cwd}[ ()]. If [argv0] is specified
      it is used instead of [cmd]'s tool for Sys.argv.(0). *)

  type t = More__cmd.t
  (** {!Exit} needs that alias to refer to {!More.Cmd.t}. *)
end

(** Program exit.

    There are two ways for a program to exit: either by returning an
    exit code or by calling {!More.Os.Cmd.execv}. The type here
...
module Exit : sig
  ...
  val execv_cwd : execv -> More__filepath.t option
  (** [execv_env exec] is the environment of [exec]. *)

  val execv_argv0 : execv -> string option
  (** [execv_env exec] is the environment of [exec]. *)

  val execv_cmd : execv -> More__cmd.t
...
module Pty : sig
  ...
  (*
  val open' :
    ?timeout:More__mtime.Span.t -> ?echo:bool ->
...
*)
  ...
  val open_with_spawn :
    ?env:Env.assignments -> ?cwd:More__filepath.t -> ?stdin:Cmd.stdi ->
    ?stdout:Cmd.stdo -> ?stderr:Cmd.stdo -> More__cmd.t ->
    (t, string) result
  ...
  val with_spawn :
    ?env:Env.assignments -> ?cwd:More__filepath.t -> ?stdin:Cmd.stdi ->
    ?stdout:Cmd.stdo -> ?stderr:Cmd.stdo -> More__cmd.t -> (t -> 'a) ->
    ('a, string) result
  ...
  val read :
    ?timeout:More__mtime.Span.t -> ?max:int -> t ->
  ...
  val seek :
    ?log:bool ->
    ?timeout:More__mtime.Span.t -> t -> string -> (unit, string) result
...
val sleep : More__mtime.Span.t -> More__mtime.Span.t
...
module Cpu : sig
  ...
  module Time : sig
    ...
    module Span : sig
      ...
      val make :
        utime:More__mtime.Span.t -> stime:More__mtime.Span.t ->
        children_utime:More__mtime.Span.t ->
        children_stime:More__mtime.Span.t -> t
      ...
      val utime : t -> More__mtime.Span.t
      (** [utime cpu] is the user time of [cpu]. *)

      val stime : t -> More__mtime.Span.t
      (** [stime cpu] is the system time of [cpu]. *)

      val children_utime : t -> More__mtime.Span.t
      (** [children_utime cpu] is the user time for children processes
          of [cpu]. *)

      val children_stime : t -> More__mtime.Span.t
...
module Mtime : sig
  ...
  val count : counter -> More__mtime.Span.t
