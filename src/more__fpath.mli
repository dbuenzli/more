...
(** [equal p0 p1] is true iff [p0] and [p1] are stringwise equal. If
    ...
    general if you want to assert that use {!More.Os.Path.realpath} on
...
module Set : sig
  ...
  val pp : ?sep:unit More__fmt.t -> path More__fmt.t -> t More__fmt.t
...
module Map : sig
  ...
  val pp : ?sep:unit More__fmt.t -> (path * 'a) More__fmt.t -> 'a t More__fmt.t
  ...
  val pp_dump : 'a More__fmt.t -> 'a t More__fmt.t
...
val sort_by_ext : multi:bool -> Set.t -> Set.t More__string.Map.t
