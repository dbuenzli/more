...
let path p = A (More__fpath.to_string p)
...
let paths ?slip ps = of_list ?slip More__fpath.to_string ps
...
type tool = More__fpath.t

let tool = arg
let rec find_tool = function
| A a -> Result.to_option (More__fpath.of_string a)
...
| l ->
    let rec loop = function
    | A a -> A (More__fpath.to_string tool)
