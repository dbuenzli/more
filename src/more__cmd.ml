...
let path p = A (More__filepath.to_string p)
...
let paths ?slip ps = of_list ?slip More__filepath.to_string ps
...
(* Parsing is loosely based on
  ...
  try
    let err_unclosed kind _ =
      More__fmt.failwith "unclosed %s quote delimited string" kind
    in
    let skip_white s =
      More__string.drop_first_while More__char.Ascii.is_white s
    in
    let tok_sep c = c = '\'' || c = '\"' || More__char.Ascii.is_white c in
    let tok_char c = not (tok_sep c) in
    let not_squote c = c <> '\'' in
    let tail s = (* Yikes *) More__string.subrange ~first:1 s in
    let parse_squoted s =
      let tok, rem = More__string.cut_first_while not_squote (tail s) in
    ...
    let rec loop acc s =
      let data, rem = More__string.cut_first_while is_data s in
      match More__string.head rem with
      | Some '"' -> (data :: acc), (tail rem)
      | Some '\\' ->
          let rem = tail rem in
          begin match More__string.head rem with
          | Some ('"' | '\\' | '$' | '`' as c) ->
              let acc = (More__string.of_char c) :: data :: acc in
              loop acc (tail rem)
          | Some ('\n') -> loop (data :: acc) (tail rem)
          | Some c ->
              let acc = (data ^ (More__fmt.str "\\%c" c)) :: acc in
              loop acc (tail rem)
          | None ->
    ...
    let parse_token s =
      ...
      let rec loop acc s = match More__string.head s with
      | None -> ret acc s
      | Some c when More__char.Ascii.is_white c -> ret acc s
      | Some '\'' ->
      ...
      | Some c ->
          let sat = tok_char in
          let tok, rem = More__string.cut_first_while sat s in
          loop (tok :: acc) rem
      in
      loop [] s
    in
  ...
  with Failure err ->
    More__fmt.error "command line %a: %s" More__fmt.OCaml.string s err
...
let pp ppf l =
  More__fmt.pf ppf "@[%a@]" More__fmt.(list ~sep:sp string) (to_list l)

let pp_arg ppf a = More__string.pp ppf (maybe_quote a)
let pp_dump ppf l =
  More__fmt.pf ppf "@[<h>%a@]" More__fmt.(list ~sep:sp pp_arg) (to_list l)
...
type tool = More__filepath.t

let tool = arg
let rec find_tool = function
| A a -> Result.to_option (More__filepath.of_string a)
...
| None -> More__fmt.error "%s: Not a tool" (to_string l)

let rec set_tool tool = function
| Rseq [] -> path tool
| l ->
    let rec loop = function
    | A a -> A (More__filepath.to_string tool)
